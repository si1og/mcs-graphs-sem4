...
Для хранения матриц используется класс \texttt{Matrix}, приведённый в листинге~\ref{lst:matrix-storage}. Он содержит количество строк и столбцов, а также двумерный вектор вещественных значений. Доступ к элементам выполняется по индексам строки и столбца.

\codeListing[cpp][firstline=8,lastline=37,breaklines]{../src/matrix.h}{Класс хранения матрицы}{\label{lst:matrix-storage}}

Граф представлен классом \texttt{Graph}, приведённым в листинге~\ref{lst:graph-storage}. В нём хранятся количество вершин, матрицы смежности и весов ориентированного графа, а также соответствующие матрицы неориентированного графа. Списки смежности и взвешенные списки смежности формируются из этих матриц при обращении к соответствующим методам.

\codeListing[cpp][firstline=7,lastline=46,breaklines]{../src/graph.h}{Структуры хранения графа}{\label{lst:graph-storage}}

\newpage

Параметры распределения Вейбулла хранятся в структуре \texttt{WeibullParams}, приведённой в листинге~\ref{lst:weibull-params}. Поля \texttt{scale}, \texttt{shape} и \texttt{shift} соответствуют масштабу, форме и сдвигу распределения. Для генерации графа, весов, пропускных способностей и стоимостей заданы отдельные наборы параметров.

\codeListing[cpp][firstline=9,lastline=41,breaklines]{../src/constants.h}{Структура и наборы параметров распределения Вейбулла}{\label{lst:weibull-params}}
...
Алгоритм Флери реализован методом \texttt{m\_flueryAlgorithm}, приведённым в листинге~\ref{lst:fleury-implementation}. Он последовательно выбирает соседнюю вершину, удаляет пройденное ребро и добавляет вершину в результат после исчерпания её соседей.
